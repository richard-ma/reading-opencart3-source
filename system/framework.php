<?php
// Registry
$registry = new Registry(); // 这是保存全局变量的类对象

// Config
$config = new Config();
$config->load('default');
$config->load($application_config); // 加载catalog类型的配置
$registry->set('config', $config); // 将配置作为全局变量注册
...
set_error_handler(function($code, $message, $file, $line) use($log, $config) {
	...
	if ($config->get('error_display')) { // 直接在页面显示错误
		echo '<b>' . $error . '</b>: ' . $message . ' in <b>' . $file . '</b> on line <b>' . $line . '</b>';
	}

	if ($config->get('error_log')) { // 输出到error_log
