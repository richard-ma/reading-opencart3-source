...
final class DB {
	...
	public function read($session_id) { // 从session表读取对应session的数据
	...
	public function write($session_id, $data) {
		if ($session_id) { // 保存数据到数据库，如果存在则替换
	...
	public function destroy($session_id) { // 删除数据库记录
		$this->db->query("DELETE FROM `" . DB_PREFIX . "session` WHERE session_id = '" . $this->db->escape($session_id) . "'");
		
		return true;
	}
	
	public function gc($expire) { // session超过expire则过期，进行gc回收，删除记录
