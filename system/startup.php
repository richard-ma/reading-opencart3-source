...
function modification($filename) {
	if (defined('DIR_CATALOG')) {
		$file = DIR_MODIFICATION . 'admin/' .  substr($filename, strlen(DIR_APPLICATION)); // 截取文件名并更换目录名称
...
function library($class) {
	$file = DIR_SYSTEM . 'library/' . str_replace('\\', '/', strtolower($class)) . '.php'; // 将类名转换为library下的目录路径

	if (is_file($file)) {
		include_once(modification($file)); // 加载类文件
...
spl_autoload_register('library'); // 注册自动加载函数
